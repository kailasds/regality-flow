...
const processedStatuses = ["Processed", "Under Review"];

export default function ProcessedNotifications() {
  const navigate = useNavigate();
  const filtered = notifications.filter((n) => n.stage === "Pending" && processedStatuses.includes(n.subStatus));

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-foreground">Processed Notifications</h2>
        <p className="text-sm text-muted-foreground mt-1">Notifications that completed AI processing</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg card-glow">
        <table className="w-full">
          ...
          <tbody>
            {filtered.map((n) => (
              <tr key={n.id} className="border-b border-border last:border-0 hover:bg-secondary/50 cursor-pointer transition-colors"
                ...
                <td className="px-5 py-3.5">
                  <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-success/15 text-success">{n.subStatus}</span>
                </td>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-muted-foreground text-sm">No processed notifications</td></tr>
            )}
          </tbody>
        </table>
      </div>
