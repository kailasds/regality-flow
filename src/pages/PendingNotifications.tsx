...
const pendingStatuses = ["Processing", "Under Review", "For Action"];

export default function PendingNotifications() {
  const navigate = useNavigate();
  const filtered = notifications.filter((n) => pendingStatuses.includes(n.subStatus));
      ...
      <div className="bg-card border border-border rounded-lg card-glow">
        <table className="w-full">
          ...
          <tbody>
            {filtered.map((n) => (
              <tr key={n.id} className="border-b border-border last:border-0 hover:bg-secondary/50 cursor-pointer transition-colors"
                ...
                <td className="px-5 py-3.5">
                  <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-primary/15 text-primary">{n.subStatus}</span>
                </td>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-muted-foreground text-sm">No pending notifications</td></tr>
            )}
          </tbody>
        </table>
      </div>
